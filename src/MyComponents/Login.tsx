...
export default function Login() {
  ...
  const validationSchema = Yup.object().shape({
    emailOrPhone: Yup.string()
      .test(
        "email-or-phone",
        t("login.validationSchema.email.invalid"),
        (value) => {
      ...
      .required(t("login.validationSchema.email.required")),
    password: Yup.string()
      .min(8, t("login.validationSchema.password.matches.message"))
      .required(t("login.validationSchema.password.required")),
  });
  ...
  async function handleLogIn(formValues: ILogInForm) {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        ...
        setAlert({
          message: error.response.data.message || t("errors.login_error"),
          type: "error",
        });
        setTimeout(() => {
          setAlert(null);
  ...
  async function sendOtp() {
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          navigate(`/access-account/${values.emailOrPhone}`);
          setIsLoading(false);
        }, 3000);
      }
    } catch (error) {
        ...
        <Card className="w-full max-w-md p-6">
            ...
            <div className="grid gap-4">
              <InputAnimation>
                <div className="grid gap-2">
                  <Label htmlFor="email">{t("login.email_or_phone")}</Label>
                  <div className="relative">
              ...
              <div className="text-right">
                <Link
                  to="/forgot-password"
                  className="text-sm text-primary hover:underline btn font-medium"
                >
          ...
          <div className="grid gap-2">
            <Button variant="outline" className="w-full btn font-medium">
              <img src={google} alt="Google" className="mr-2 h-6 w-6" />
