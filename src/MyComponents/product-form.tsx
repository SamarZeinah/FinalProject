...
const{t}=useTranslation();
  ...
  const validationSchema = Yup.object({
    ...
    price: Yup.string()
      .required(t("ProductForm.price"))
      .test(t("ProductForm.valid-price"),t("ProductForm.Price-must-number"), (value) => {
        return !isNaN(Number(value))
      }),
    baseUnit: Yup.string().required(t("ProductForm.baseUnit")),
    descriptionEn: Yup.string().required(t("ProductForm.descriptionEn")),
    descriptionAr: Yup.string().required(t("ProductForm.descriptionAr")),
    length: Yup.string().test(t("ProductForm.valid-length"),t("ProductForm.length-must-number"), (value) => {
      return !value || !isNaN(Number(value))
    }),
    width: Yup.string().test(t("ProductForm.valid-width"),t("ProductForm.width-must-number"), (value) => {
      return !value || !isNaN(Number(value))
    }),
    height: Yup.string().test(t("ProductForm.valid-height"),t("ProductForm.height-must-number"), (value) => {
      return !value || !isNaN(Number(value))
    }),
  })
  ...
  async function handleSubmit(values: ProductFormValues) {
    ...
    if (imagePreviewUrls.length === 0 && existingImagePaths.length === 0) {
      setAlert({
        message:t("ProductForm.Please-upload-least-one"),
        type: "error",
      })
    ...
    try {
      ...
      setAlert({
        message: isEditMode ? t("ProductForm.Product-updated-successfully") : t("ProductForm.Product-added-successfully"),
        type: "success",
      })
      ...
      setAlert({
        message: isEditMode
          ? t("ProductForm.Failed-updatey")
          : t("ProductForm.Failed-add"),
        type: "error",
      })
      setTimeout(() => setAlert(null), 5000)
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <span className="ml-2">{t("ProductForm.Loading-product-data")}</span>
      </div>
    )
  }
              ...
              <Button type="submit" className="bg-[#4f5d95] hover:bg-[#3f4d85] text-white px-8" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    ...
                    {isEditMode ?t("ProductForm.Updating") : t("ProductForm.submitting")}
                  </>
                ) : isEditMode ? (
                  t("ProductForm.Update-Product")
                ) : (
