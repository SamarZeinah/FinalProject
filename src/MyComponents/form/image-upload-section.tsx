...
const{t}=useTranslation()
  return (
    <div className="space-y-2">
      <Label>{t("ImageUploadSection.Product-Images")}</Label>
      {!hasImages ? (
        <div
        ...
        >
          ...
          <p className="text-sm font-medium text-gray-900">{t("ImageUploadSection.Upload-Image")}</p>
          <p className="text-xs text-gray-500">{t("ImageUploadSection.drop-drag")}</p>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-700">{t("ImageUploadSection.Add-More-Images")}</span>
          </div>
          ...
          <div className="space-y-3">
            ...
            {isEditMode && existingImageFiles.length > 0 && (
              <div className="text-sm font-medium text-gray-700 border-b pb-2">{t("ImageUploadSection.Existing-Images")}</div>
            )}
            ...
            {newImageFiles.length > 0 && existingImageFiles.length > 0 && (
              <div className="text-sm font-medium text-gray-700 border-b pb-2 mt-4">{t("ImageUploadSection.New-Images")}</div>
            )}
