...
const ProductDetails: React.FC<ProductDetailsProps> = ({ productId, onClose, onDelete }) => {
  ...
  const [selectedTab, setSelectedTab] = useState(i18n.language === "ar" ? "التفاصيل" : "details");
    ...
    return (
      ...
      >
        <div className="bg-white rounded-2xl p-8 flex flex-col items-center">
          ...
          <p className="text-gray-600 mt-4 font-medium">{t("ProductDetails.Loading-product")}</p>
        </div>
      </motion.div>
    )
  }
      ...
      >
        <div className="bg-white rounded-2xl p-8 max-w-md w-full">
          <div className="flex items-center text-red-500 mb-4">
            ...
            <h3 className="text-lg font-semibold">{t("ProductDetails.Error-Loading")}</h3>
          </div>
          <p className="text-gray-600 mb-6">{t("ProductDetails.Unable-load")}</p>
          ...
          <div className="p-0 lg:max-h-[90vh] lg:overflow-y-auto relative bg-white" ref={detailsRef}>
            ...
            <div className="sticky top-0 z-30 bg-white border-b border-gray-100 px-6 py-3">
              <div className="flex space-x-1 overflow-x-auto scrollbar-hide">
                {[t("ProductDetails.details"),t("ProductDetails.colors"),t("ProductDetails.materials")].map((tab) => (
                  <button
                    key={tab}
              ...
              <div>
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-full">
                    ...
                    <span className="text-gray-700 text-sm font-medium">{t("ProductCard.Stock")}: {totalStock}</span>
                  </div>
                </div>
              </div>

              <AnimatePresence mode="wait">
                {selectedTab === t("ProductDetails.details") && (
                  <motion.div
                    key="details"
                  ...
                  >
                    <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-5 rounded-xl border border-gray-200">
                      ...
                      <div className="grid grid-cols-3 gap-4">
                        <div className="bg-white p-4 rounded-lg shadow-sm text-center">
                          ...
                          <div className="text-sm text-gray-500">{t("ProductDetails.Length")} ({product.baseUnit.name})</div>
                        </div>
                        <div className="bg-white p-4 rounded-lg shadow-sm text-center">
                          <div className="text-lg font-semibold text-gray-900">{product.width}</div>
                          <div className="text-sm text-gray-500">{t("ProductDetails.Width")}  ({product.baseUnit.name})</div>
                        </div>
                        <div className="bg-white p-4 rounded-lg shadow-sm text-center">
                          <div className="text-lg font-semibold text-gray-900">{product.height}</div>
                          <div className="text-sm text-gray-500">{t("ProductDetails.Height")} ({product.baseUnit.name})</div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-4">
                      <Button
                        ...
                        onClick={() => setSelectedTab(t("ProductDetails.materials"))}

                      >
                        {t("ProductDetails.View-Materials")} 
                ...
                {selectedTab === t("ProductDetails.colors") && (
                  <motion.div
                    key="colors"
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-5 rounded-xl border border-gray-200">
                      <h3 className="text-sm font-medium text-gray-900 mb-4 flex items-center">
                        ...
                        {t("ProductDetails.Available-Colors")}
                      </h3>
                            ...
                            <div className="flex flex-col items-start">
                              ...
                              <div className="flex items-center gap-1 mt-1">
                                ...
                                <span className="text-sm text-gray-500">
                                  {stock.amount > 0 ? `${stock.amount} ${t("ProductDetails.in-stock")}` : t("ProductDetails.Out-Stock")}
                                </span>
                              </div>
                            </div>
                          </motion.button>
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg border border-gray-200">
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <p>{t("ProductDetails.Select-color")}</p>
                      </div>
                    </div>
                  </motion.div>
                )}

                {selectedTab ===t("ProductDetails.materials") && (
                  <motion.div
                    key="materials"
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                        ...
                        {product.materials.map((material) => (
                          ...
                          >
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-500">{t("ProductDetails.Code")}: {material.code}</p>
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          <h4 className="font-medium text-amber-800 mb-1">{t("ProductDetails.Premium Materials")}</h4>
                          <p className="text-sm text-amber-700">
                          {t("ProductDetails.Materials-description")}
                          </p>
                        </div>
                      </div>
                    </div>
