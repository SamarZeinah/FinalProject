import { ChevronLeft, ChevronRight, HandCoins, RotateCcw, Star, Truck } from "lucide-react";
    ...
    <div className="space-y-6 p-6">
        ...
        <div className="flex flex-col">
          ...
          <div className="text-gray-800 text-sm font-medium   inline-flex items-center justify-center space-x-1 text-center">
            <span className="bg-gray-800 text-white text-xs font-bold px-3 py-2 rounded-full">
             {t('ProductsDetails.Best-Seller')}
            </span>

            <span className="px-[4px]"> {t('ProductsDetails.in')} {product.category}</span>
           { i18n.language === "ar" ? (
      ...
      <div className="flex items-center space-x-2 mb-4">
        <span className="text-xl font-bold text-gray-800">
            {t('ProductsDetails.EGP')} {product.price.toFixed(2)}
        </span>
        <span className="text-sm line-through text-gray-500">
            {t('ProductsDetails.EGP')} {product.originalPrice.toFixed(2)}
        </span>
        <p className="text-sm text-gray-500">{t('ProductsDetails.Inclusive-of-VAT')}</p>
      </div>

      {/* Savings and Discount Percentage */}
      <div className="flex items-center space-x-2 mb-4">
        <span className="text-green-600 font-semibold">
           {t('ProductsDetails.Saving:EGP')} {product.saving.toFixed(2)}
        </span>
        <span className="text-green-600 font-semibold">
          ({product.discountPercentage}%   {t('ProductsDetails.Off')})
        </span>
      </div>
      ...
      <div className="flex items-center space-x-2 text-sm text-gray-600 mb-4">
        ...
        <span>
          #{product.rank}{t('ProductsDetails.in')}{" "}
      ...
      <div className="border rounded-lg p-4 flex justify-between text-center mb-6">
        <div className="flex flex-col items-center space-y-1">
          ...
          <span className="text-xs text-gray-800">{t('ProductsDetails.Delivery-by-ArciSpace')}</span>
        </div>
        <div className="flex flex-col items-center space-y-1">
          <Star className="w-6 h-6 text-yellow-500" />
          <span className="text-xs text-gray-800">{t('ProductsDetails.High-Rated-Seller')}</span>
        </div>
        <div className="flex flex-col items-center space-y-1">
          <RotateCcw className="w-6 h-6 text-green-600" />
          <span className="text-xs text-gray-800">{t('ProductsDetails.Low-Returns')}</span>
        </div>
        <div className="flex flex-col items-center space-y-1">
          <HandCoins className="w-6 h-6 text-purple-600" />
          <span className="text-xs text-gray-800">{t('ProductsDetails.Cash-Delivery')}</span>
        </div>
      </div>
      ...
      <div className="border border-gray-300 rounded-lg p-3 hover:border-gray-900">
        ...
        >
          ...
          <span className="text-sm text-gray-800 font-medium flex-grow ml-2">
           {t('ProductsDetails.Ranked')} #{product.rank}{t('ProductsDetails.in')}{" "}
            <span className="font-semibold">{product.category}</span>
            <br />
            {t('ProductsDetails.Explore-bestsellers')}
          </span>
