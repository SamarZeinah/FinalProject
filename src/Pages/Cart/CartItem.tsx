    ...
    return (
      ...
      >
        <div className="flex gap-4 items-center justify-center py-8">
          ...
          <span className="text-gray-600">{t('CartItem.Loading product...')}</span>
        </div>
      </motion.div>
    )
  }
    ...
    return (
      ...
      >
        <div className="flex gap-4 items-center justify-center py-8">
          ...
          <span className="text-red-600">{t('CartItem.Error-loading-product')}</span>
          <Button variant="outline" size="sm" onClick={handleRemoveFromCart} className="ml-4">
            {t('CartItem.Remove-from-cart')}
          </Button>
        </div>
      </motion.div>
    )
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
            ...
            <div className="flex flex-row sm:flex-col items-start sm:items-end justify-between sm:justify-start gap-2 sm:gap-2 flex-shrink-0 order-0 sm:order-none">
              ...
              >
                {t('CartItem.EGP')} {(product.price * quantity).toLocaleString()}
              </motion.div>
              ...
              <div className="flex flex-row sm:flex-col items-center sm:items-end gap-2 sm:gap-1">
                ...
                >
                  ...
                  <span> {t('CartItem.Free-Delivery')}</span>
                </motion.div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <p>
              {t('CartItem.Get-it-by')} <span className="font-medium text-green-600">{getDeliveryDate()}</span>
            </p>
            <p className="text-blue-600">
              {t('CartItem.Business-Type')}  <span className="font-medium">{product.businessType.name}</span>
            </p>
          </motion.div>
          ...
          >
            ...
            >
              ...
              <span>{t('CartItem.1year-warranty')} </span>
            </motion.div>

            {/* Dimensions */}
            <div className="text-xs text-gray-500">
              {t('CartItem.Dimensions')}  {product.length} × {product.width} × {product.height} {product.baseUnit.name}
            </div>
          </motion.div>
          ...
          <div className="flex sm:flex-row items-center justify-between mt-3 md:mt-4 pt-3 md:pt-4 border-t border-gray-100 gap-3 sm:gap-0">
            ...
            <div className="flex items-center gap-2 md:gap-3 order-2 sm:order-1">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t('CartItem.Move-to-Wishlist')} </span>
                  <span className="sm:hidden">{t('CartItem.Wishlist')} </span>
                </Button>
              </motion.div>
            </div>
            ...
            <div className="flex items-center justify-between sm:justify-end gap-3 order-1 sm:order-2">
              <div className="flex items-center gap-2">
                <span className="text-xs md:text-sm text-gray-500">{t('CartItem.Qty')} </span>
