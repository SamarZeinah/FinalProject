...
export default function ColorsAndStockSection({
  ...
  const handleAddColor = () => {
    ...
    if (!selectedColor) {
      setColorError(t("ColorsAndStockSection.Please-select-color"))
      return
    }

    if (!stockValue) {
      setColorError(t("ColorsAndStockSection.Please-enter-stock-quantity"))
      return
    }
    ...
    if (isNaN(stockNum)) {
      setColorError(t("ColorsAndStockSection.Stock-must-number"))
      return
    }

    if (stockNum < 0) {
      setColorError(t("ColorsAndStockSection.Stock-cannot-negative"))
      return
    }
    ...
    if (colorExists) {
      setColorError(t("ColorsAndStockSection.color-already-added"))
      return
    }
...
const{t}=useTranslation()
  return (
    <div className="space-y-2">
      <Label>{t("ColorsAndStockSection.Colors-and-Stock")}</Label>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
        <div className="relative">
          ...
          >
            <SelectTrigger className={`w-full border-[#e5e7eb] ${colorError ? "border-red-500" : ""}`}>
              <SelectValue placeholder={t("ColorsAndStockSection.Colors")} />
            </SelectTrigger>
            <SelectContent>
        ...
        <div className="flex gap-2">
          <Input
            placeholder={t("ColorsAndStockSection.Stock")}
            value={stockValue}
