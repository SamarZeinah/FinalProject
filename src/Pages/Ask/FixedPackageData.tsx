          ...
          {data.map((item, index) => (
            <div key={index}>
              <div className="max-h-[420px] overflow-y-scroll scrollbar-hide p-4 rounded-md">
                <h3 className="mb-3">
                  <span className="font-bold">{t('FixedPackage.Package-Name:')}</span>
                  <span className="text-gray-600">{item?.name}</span>
                </h3>
                <h3 className="mb-4">
                  <span className="font-bold">{t('FixedPackage.Price:')}</span>
                  <span className="text-gray-600"> {item.price} {t('FixedPackage.EGP')}</span>
                </h3>

                <div className="text-gray-700">
                  {item?.details.split("<h3>").map((block, idx) =>
                    block ? (
          ...
          >
            {isLoading ? (
              <>
                ...
                {t('FixedPackage.Booking...')}
              </>
            ) : (
              t('FixedPackage.Book-the-Package')
            )}
          </Button>
        </div>
      </div>
...
//       setAlert({
//         message: response.data?.message || "Package booked successfully!",
//         type: "success",
//       });
...
//             const details = currentLang === "ar" ? item.detailsAr : item.detailsEn;
...
//                   <div className="text-gray-700">
//                     {details.split("<h3>").map((block, idx) =>
//                       block ? (
//                         <div key={idx} className="mb-2">
//                           <h3 className="font-bold mb-1 text-gray-800">
//                             {block.split("</h3>")[0]}
