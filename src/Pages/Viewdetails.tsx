...
const{i18n}=useTranslation();
      ...
      <div className="container mx-auto py-4 px-6 max-w-full">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-gray-600">{t('viewdetails.Loading-product-details')}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="container mx-auto py-4 px-6 max-w-full">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            {t('viewdetails.Product-Not-Found')}
            </h2>
            <p className="text-gray-600">
              {typeof error === "string"
                ? error
                : t('viewdetails.The-product-looking')}
            </p>
          </div>
        </div>
      </div>
  ...
  const transformedOverview = {
    description:
      ...
      t('viewdetails.No-description-available'),
   highlights: [
  `${t('viewdetails.Made-high-quality')}, ${productData.materials?.map((m) => m.name).join(", ") || t('viewdetails.Various-materials')}`,
  `${t('viewdetails.Dimensions')}: ${productData.length}x${productData.width}x${productData.height} ${productData.baseUnit?.name || t('viewdetails.units')}`,
  `${t('viewdetails.Category')}: ${productData.businessType?.name || t('viewdetails.General')}`,
  t('viewdetails.High-quality'),
],

    specifications: [
      [t('viewdetails.Product-ID'), productData.id?.toString() ?? "N/A"] as [string, string],
      [t('viewdetails.Category'), productData.businessType?.name || "N/A"] as [string, string],
      [
        t('viewdetails.Materials'),
        productData.materials?.map((m) => m.name).join(", ") || "N/A",
      ] as [string, string],
      [
        t('viewdetails.Dimensions'),
        `${productData.length} x ${productData.width} x ${productData.height} ${
          productData.baseUnit?.name || ""
        }`,
      ] as [string, string],
      [
       t('viewdetails.Available-Colors'),
        productData.stocks?.map((s) => s.color.name).join(", ") || "N/A",
      ] as [string, string],
      [
        t('viewdetails.Total-Stock'),
        productData.stocks
