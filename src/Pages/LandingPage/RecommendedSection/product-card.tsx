...
export default function ProductCard({ product }: { product: ApiProduct }) {
  ...
  const animatedTexts = [
    ...
    {
text: product.numberOfSales
  ? `${product.numberOfSales}+ ${t('Product-Card.sold-recently')}`
  : t('Product-Card.sold-recently'),
      ...
      {product.bestSeller && (
        <div className="absolute top-3 left-3 z-10">
          <Badge className="bg-gray-700 text-white text-xs font-medium px-2 py-1 rounded-full">
            {product.bestSeller ?t('Product-Card.Best-Seller') : t('Product-Card.Featured')}
          </Badge>
        </div>
      )}
        ...
        <div className="p-4 border-t border-gray-100">
          ...
          <div className="flex items-center mb-3">
            <div className="flex items-baseline">
              <span className="text-sm font-medium">{t('Product-Card.EGP')}</span>
