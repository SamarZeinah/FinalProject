...
const useRatingChart = (productId: number) => {
  ...
  useEffect(() => {
    ...
    fetchRatingChart()
  }, [pathUrl, productId, userToken,i18n.language])
...
const ProductReviews = ({ productId }: ProductReviewsProps) => {
  ...
  return (
    <div className="bg-white border border-gray-200 shadow-md rounded-lg px-6 pb-[80px] pt-[20px]">
      <h1 className="text-2xl font-bold text-gray-800 border-b pb-2 mb-6">{t('ProductsReviews.Product-Ratings')}</h1>
      <div className="flex flex-col md:flex-row gap-24">
        <div className="w-full md:w-1/3 flex flex-col gap-4">
          <h1 className="text-xl font-bold text-gray-800">{t('ProductsReviews.Overall-Rating')}</h1>

          {chartLoading ? (
            <div className="flex justify-center py-8">
              <div className="text-gray-500">{t('ProductsReviews."Loading-rating-data...')}</div>
            </div>
          ) : chartError ? (
            <div className="text-center text-red-600">
              <p className="text-sm">{t('ProductsReviews.Error-loading-rating-data')}</p>
            </div>
          ) : (
            <>
              <div className="flex flex-col">
              ...
              <p className="text-sm text-gray-600">
                {t('ProductsReviews.Based-on')} {ratingChart?.countRantings || 0} {t('ProductsReviews.ratings')}{(ratingChart?.countRantings || 0) !== 1 ? "s" : ""}
              </p>
        ...
        <div className="w-full lg:w-2/3 space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
            <h2 className="text-xl font-semibold">{t('ProductsReviews.Reviews')} ({totalElements})</h2>
            <div className="flex gap-2">
              <div className="flex items-center gap-2">
                <Label htmlFor="filter-rating" className="text-sm font-bold whitespace-nowrap">
                {t('ProductsReviews.Filter-By:')}
                </Label>
                ...
                >
                  <SelectTrigger>
                    <SelectValue>
                      {filterStars === null ? (
                        <span>{t('ProductsReviews.All-stars')}</span>
                      ) : (
                  ...
                  <SelectContent>
                    <SelectItem value="0">{t('ProductsReviews.All-stars')}</SelectItem>
                    {[...Array(5)].map((_, index) => (
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="top">{t('ProductsReviews.Top-Reviews')}</SelectItem>
                    <SelectItem value="newest">{t('ProductsReviews.Newest')}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          {loading && reviews.length === 0 ? (
            <div className="flex justify-center py-8">
              <div className="text-gray-500">{t('ProductsReviews.Loading-reviews...')}</div>
            </div>
          ) : reviews.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500">{t('ProductsReviews.No-reviews-found')}</p>
            </div>
          ) : (
            <>
                ...
                <div className="flex justify-center pt-6">
                  <Button onClick={loadMore} disabled={loading} variant="outline" className="flex items-center gap-2">
                    {loading ? (
                      ...
                      <>
                        {t('ProductsReviews.Load-More-Reviews')}
