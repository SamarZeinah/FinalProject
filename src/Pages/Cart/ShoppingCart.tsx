            ...
            >
              <div className="p-6 border-b flex justify-between items-center">
                <h1 className="text-2xl font-semibold text-gray-900">
                  ...
                  >
                    ({cartData.cartProducts?.length || 0} {t('ShoppingCart.items')})
                  </motion.span>
                </h1>
                ...
                cartData.cartProducts.length === 0 ? (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">
                      {t('ShoppingCart.Your-cart-empty')}
                    </h3>
                    <p className="text-gray-500">
                      {t('ShoppingCart.Add-some-products')}
                    </p>
                  </motion.div>
                ) : (
              ...
              <Card className="sticky top-6">
                <CardContent className="p-6">
                  ...
                  >
                    ...
                    >
                      {t('ShoppingCart.View-Available-Offers')}
                    </Button>
                  </motion.div>
                  ...
                  >
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">
                        {t('ShoppingCart.Subtotal')} ({itemCount} {t('ShoppingCart.items')})
                      </span>
                      <motion.span
                      ...
                      >
                        ...
                        {t('ShoppingCart.EGP')} {subtotal.toLocaleString()}.00
                      </motion.span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">{t('ShoppingCart.Shipping-Fee')}</span>
                      <span className="font-medium">{t('ShoppingCart.EGP')} {shippingFee}.00</span>
                    </div>
                    <Separator />
                    <div className="flex justify-between text-lg font-bold">
                      ...
                      >
                        ...
                        {t('ShoppingCart.EGP')} {total.toLocaleString()}.00
                      </motion.span>
                    </div>
                    {hasErrors && (
                      <div className="text-xs text-orange-600 flex items-center gap-1">
                        <AlertCircle className="w-3 h-3" />
                        {t('ShoppingCart.Some-items')}
                      </div>
                    )}
                  </motion.div>
                  ...
                  >
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-gray-700">
                        {t('ShoppingCart.Monthly-payment')}
                      </span>
                      <Button
                        variant="link"
                        className="text-orange-600 p-0 h-auto text-sm"
                      >
                        {t('ShoppingCart.View-more-details')}
                      </Button>
                    </div>
                  </motion.div>
                    ...
                    >
                      {isProcessingOrder ? (
                        <div className="flex items-center gap-2">
                          <Loader2 className="w-4 h-4 animate-spin" />
                          {t('ShoppingCart.Processing-Order...')}
                        </div>
                      ) : isCalculating &&
                        cartData.cartProducts &&
                        cartData.cartProducts.length > 0 ? (
