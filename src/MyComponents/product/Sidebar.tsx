...
const Sidebar: React.FC<SidebarProps> = ({ isOpen,totalProducts }) => {
  ...
  const menuItems = [
    { icon: Home, label: t("ProductSidebar.Dashboard"), href: "#" },
    { icon: Package, label: t("ProductSidebar.Products"), href: "/productlist", badge: totalProducts },
    { icon: Tag, label: t("ProductSidebar.Offers"), href: "#", badge: "New" },
    { icon: Star, label:  t("ProductSidebar.Reviews"), href: "/reviews" },
    { icon: UserRoundCog, label: t("ProductSidebar.Overview"), href: "/overview" },
    { icon: Settings, label: t("ProductSidebar.Settings"), href: "#" },
    { icon: HelpCircle, label: t("ProductSidebar.Help-Center"), href: "#" },
  ]
  return (
    <motion.div
      initial={false}
    ...
    >
        ...
        <div className="flex items-center justify-between mb-8">
          {isOpen ? (
            ...
            >
              ...
              <span className="text-xl font-semibold"> {t("ProductSidebar.ArciSpace")}</span>
            </motion.div>
          ) : (
            <motion.div
              initial={false}
      ...
      {isOpen ? (
        <div className="mt-auto p-6">
          ...
          >
            <h4 className="text-purple-400 font-medium mb-2"> {t("ProductSidebar.Need-Help")}</h4>
            <p className="text-sm text-gray-300 mb-3">{t("ProductSidebar.contact")}</p>
            <button className="w-full bg-purple-500 hover:bg-purple-600 text-white rounded-lg py-2 text-sm transition-colors">
            {t("ProductSidebar.Contact-Support")}
            </button>
          </motion.div>
        </div>
      ) : (
